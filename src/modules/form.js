 ...
 *
 * Validators return `true` when valid, or an error message string when invalid.
 * validateField runs validators in order; returns `true` or the first error string,
 * and shows the message in a `.field-error` sibling after the control.
 * validateForm runs validateField for each named field in fieldRules.
 * isFormSatisfiesRules checks the same rules without updating the DOM (for disabling submit, etc.).
 */
